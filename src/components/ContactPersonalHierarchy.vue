...
import {
  buildPersonalReportingTree,
  hasPersonalReportingLine,
  loadPersonalHierarchyContacts,
...
const hasReportingLine = computed(() => hasPersonalReportingLine(props.contactId, contacts.value))
const root = computed(() =>
  buildPersonalReportingTree(
    props.contactId,
    contacts.value,
...
async function loadHierarchy() {
  ...
  try {
    const result = await loadPersonalHierarchyContacts(props.contactId)
    contacts.value = result.contacts
    externalManagerByContactId.value = result.externalManagerByContactId
  } catch (loadError) {
    contacts.value = []
    externalManagerByContactId.value = new Map()
    error.value = loadError instanceof Error ? loadError.message : 'Could not load reporting lines.'
  } finally {
    loading.value = false
  }
}
...
<template>
  <div>
    ...
    <p
      v-else-if="!hasReportingLine"
      class="rounded-xl border border-stone-200 bg-stone-50 px-4 py-8 text-center text-sm text-stone-600"
    >
      No reporting lines have been recorded for this contact.
    </p>

    <div v-else-if="root" class="org-tree-shell">
      <div class="org-tree-canvas">
        <ul class="org-tree">
          <ContactHierarchyBranch
            :node="root"
            :focus-contact-id="contactId"
            is-root
          />
        </ul>
      </div>
...
.org-tree :deep(.org-node-focused) {
  border-color: #0c4a6e;
  background: #f0f9ff;
  box-shadow: 0 0 0 2px rgb(12 74 110 / 0.12);
}
